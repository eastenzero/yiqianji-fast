...
for (const [row, fname, desc] of files) {
  ops.push({
    ...
    props: { find: `${fname}${fname}`, replace: fname },
  });
  ops.push({
    op: 'set',
    path: `/body/tbl[1]/tr[${row}]/tc[2]/p[2]`,
    props: { find: `${desc}${desc}`, replace: desc },
  });
}
...
console.log(`✅ 清理完成：${results.filter(r => r.success).length} / ${results.length}`);
